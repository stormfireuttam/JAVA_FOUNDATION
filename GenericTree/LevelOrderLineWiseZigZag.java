...
public class LevelOrderLineWiseZigZag {
    ...
    public static void levelOrderLinewiseZZ(Node node){
        ...
        mainS.push(node);
        /*
            Perform the following operations until the size of mainStack is greater than zero:
            1) Remove the top element
            2) Print out the element
            3) If level is odd, push children of the current node from left to right
                        if even, push children of the current node from right to left
               to the childStack
            After the mainStack becomes empty
            - If their are elements in the child stack set mainStack equal to childStack
            - Increment level by 1
            - After removing all the elements of the main stack take the control to the next line
         */
        int level = 1;
        while (mainS.size() > 0) {
            while (mainS.size() > 0) {
